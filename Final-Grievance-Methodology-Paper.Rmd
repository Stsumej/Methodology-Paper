...
scree(matrix1,  main = "Scree Plot of Eigenvalues after Factor \nAmong White Americans") 
```
...
p.out <- fa(matrix1, 
            ...
            max.iter = 1000,
            rotate = "varimax") #All
labels <- c("Item 1", "Item 2", "Item 3", "Item 4", "Item 5", "Item 6")
fa.diagram(p.out, main = "Grievance Factor Loadings Among All Americans")
...
fa.diagram(p.outw, main = "Grievance Factor Loadings Among White Americans")
```
...
rids, sdo, lf, natid, grieve, constant
```{r Manipulation Check-ish}
#Based on Banks (2014) Table 2.1, p. 47
g.man <- lm(grieve ~ rids + sdo + c.dlf + usid, weights=gov2$weight, data=gov2)
r.man <- lm(rids ~ sdo + c.dlf + usid + grieve, weights=gov2$weight, data=gov2)
s.man <- lm(sdo ~ c.dlf + usid + grieve + rids, weights=gov2$weight, data=gov2)
c.man <- lm(c.dlf ~ usid + grieve + rids + sdo, weights=gov2$weight, data=gov2)
n.man <- lm(usid ~ grieve + rids + sdo + c.dlf, weights=gov2$weight, data=gov2)

labels1 <- c("Racial ID Strength", "SDO", "Linked Fate", "National ID Strength", "Grievances", "Constant")
stargazer(g.man, r.man, s.man, c.man, n.man, covariate.labels = labels1, type = "html", digits = 2,  star.cutoffs = c(0.05, 0.01, 0.001), omit.stat = c("LL", "ser", "f", "adj.rsq"), dep.var.labels = "Grievance Perception", out = "mainmatrix.html")
...
labels2 <- c("Racial ID Strength", "Linked Fate", "SDO", "Education", "National ID Strength", "Gender", "Income", "7-pt Party ID", "Age", "Age-Squared", "Constant")

stargazer(cor.black, cor.white, type = "html",digits = 2,covariate.labels = labels2,  digits.extra = 0,  dep.var.labels = "Grievance Perception" ,column.labels = c("Black", "White"),star.cutoffs = c(0.05, 0.01, 0.001),  se=list(NULL), omit.stat = c("LL", "ser", "f", "adj.rsq"), single.row = TRUE, out = "gcorr.html")

```

#The Role of Grievances in National Attachment

```{r Multivariate Grievance Correlates Model}

black3 <- lm(usid ~  educ + rids + sdo +  gender +  faminc_new +   pid7 +  age + age2 + c.dlf*grieve + rids*grieve, weights = gov2$weight, data=gov2)

white3 <- lm(usid ~  educ + rids + sdo +  gender +  faminc_new + pid7 +  age + age2 + c.dlf*grieve + rids*grieve, weights = gov1$weight, data=gov1)

labels3 <- c("Education", "Racial ID Strength", "SDO", "Gender", "Income", "7-pt Party ID", "Age", "Age-Squared", "Linked Fate", "Grievances", "Linked Fate*Grievances", "Racial ID Strength*Grievances","Constant")

stargazer(black3, white3, type = "text", digits = 2, digits.extra = 0, star.cutoffs = c(0.05, 0.01, 0.001), covariate.labels = labels3, dep.var.labels = "National Attachment", column.labels = c( "Black", "White"), se=list(NULL), omit.stat = c("ser", "f", "adj.rsq"))
```
...
Z = c("Intercept", "Education", "Racial ID Strength", "SDO", "Gender", "Income", "Party ID", "Age", "Age-Squared", "Linked Fate", "Grievances", "Grievances*Linked Fate", "Racial ID Strength*Grievances")

plotreg(black3,
        custom.model.names = c(" "), 
        custom.title = "Linked Fate and Black National Attachment", 
        custom.coef.names = Z, 
        omit.coef = "Intercept",
        theme = theme_minimal(),
        center = TRUE) 
...
plotreg(white3,
        custom.model.names = c(" "), 
        custom.title = "Linked Fate and White National Attachment", 
        custom.coef.names = Z, 
        omit.coef = "Intercept",
        theme = theme_minimal(),
        center = TRUE) 
...
#c(.202, .60, .995) Linked fate mean +/- 1sd
#c(.46, .66, .86) grievance mean  +/- 1sd

out <- interflex(estimator = "binning",   treat.type = "continuous", Y = "usid", D = "c.dlf", X = "grieve", Z = c("educ", "sdo", "gender", "faminc_new", "pid7", "age", "age2", "rids"), vcov.type = "robust", data = gov2,weights = "weight", predict == T, cutoff = c(.46, .66, .86), Xdistr = "density", wald = TRUE, main = "Marginal Effects of Grievances on Black National Attachment", xlab = "Linked Fate", ylab = "Marginal Effect of Grievances on Black National Attachment")

out2 <- interflex(estimator = "linear", treat.type = "continuous", Y = "usid", D = "c.dlf", X = "grieve", Z = c("educ", "sdo", "gender", "faminc_new", "pid7", "age", "age2", "rids"), vcov.type = "robust", data = gov2,weights = "weight", predict == T, cutoff =  c(.46, .66, .86), Xdistr = "density", wald = TRUE, main = "Marginal Effects of Grievances on Black National Attachment", xlab = "Linked Fate", ylab = "Marginal Effect of Grievances on Black National Attachment")

out$tests$p.wald

predict.interflex(out, newdata = gov2, weights = gov2$weight, Xdistr = "density",  main = "The Marginal Effect of Grievances on \nPredicted Values of Black National Attachment \n with Linked Fate Fixed at Median: Interflex Approach", xlab = "Marginal Effect of Grievances Among Median Linked Fate Believers", ylab = "Expected Black National Attachment", ylim = c(-.05, 1.75))

predict.interflex(out2, newdata = gov2, weights = gov2$weight, Xdistr = "density", main = "The Marginal Effect of Grievances on \nPredicted Values of Black National Attachment \n with Linked Fate Fixed at Median: Linear Approach", xlab = "Marginal Effect of Grievances Among Median Linked Fate Believers", ylab = "Expected Black National Attachment", ylim = c(-.05, 1.75))
...
Jens Hainmueller; Jonathan Mummolo; Yiqing Xu. 2019. "How Much Should We Trust Estimates from Multiplicative Interaction Models? Simple Tools to Improve Empirical Practice." Political Analysis, Vol. 27, Iss. 2, April 2019, pp. 163--192. Available at: http://bit.ly/HMX2019.
